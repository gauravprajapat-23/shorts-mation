...
export const getBillingOverview=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((d:Record<string,never>)=>d).handler(async({context})=>{
...
export const startCheckout=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((d:{plan:BillingPlanKey;interval:"month"|"year"})=>d).handler(async({context,data})=>{
...
export const openBillingPortal=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((d:Record<string,never>)=>d).handler(async({context})=>{
...
export const setEntitlementOverride=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((d:{userId:string;planKey?:BillingPlanKey|null;overrides?:Record<string,number>;reason:string;expiresAt?:string|null})=>d).handler(async({context,data})=>{
